 ...
 */
public class CompanyListAdapter extends BXBaseAdapter<CompanyListModel.CompanyListModelDeal> implements Filterable {
    ...
    private ArrayList<CompanyListModel.CompanyListModelDeal> mOriginalValues;
    private ArrayFilter mFilter;

    public CompanyListAdapter(List listModel, Activity activity) {
        super(listModel, activity);
    }

    @Override
    public View getItemView(int position, View convertView, ViewGroup parent, CompanyListModel.CompanyListModelDeal model) {
        if (convertView == null) {
    ...
    private class ArrayFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence prefix) {
            ...
            if (prefix == null || prefix.length() == 0) {
                ArrayList<CompanyListModel.CompanyListModelDeal> list;
                ...
                ArrayList<CompanyListModel.CompanyListModelDeal> values;
                synchronized (mLock) {//同步复制一个原始备份数据
                    values = new ArrayList<>(mOriginalValues);
                }
                final int count = values.size();
                final ArrayList<CompanyListModel.CompanyListModelDeal> newValues = new ArrayList<>();

                for (int i = 0; i < count; i++) {
                    final CompanyListModel.CompanyListModelDeal value = values.get(i);//从List<User>中拿到User对象
        ...
        protected void publishResults(CharSequence prefix, FilterResults results) {
            ...
            mListModel = (List<CompanyListModel.CompanyListModelDeal>) results.values;//此时，Adapter数据源就是过滤后的Results
            if (results.count > 0) {
